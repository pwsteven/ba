...
let wieldy = {
    ...
    sidebar: {
        ...
        insertOverlay: function () {
            ...
            let drawer = this;
            let overlayContainer = $(this.overlay);
    ...
    hoverCard: {
        ...
        showThumb: function () {
            let bodyWidth = this.docBody.outerWidth(true);

            if (bodyWidth > 767) {
                let $this = this;
                let offset = this.handleRef.offset();
                let handleWidth = this.handleRef.outerWidth(true);
                let name = (this.handleRef.data('name')) ? this.handleRef.data('name') : '';

                let innerHtml = '<span class="user-bg-card__info"><span class="dt-avatar-name text-center">' + name + '</span></span>';
...
(function ($) {
    let $body = $('body');
    let $loader = $('.dt-loader-container');
    let $root = $('.dt-root');
    ...
    /* customizer settings */
    let $dtTheme = localStorage.getItem('dt-theme');
    let $dtLayout = localStorage.getItem('dt-layout');
    let $dtStyle = localStorage.getItem('dt-style');
    ...
    let $themeChooser = $('#theme-chooser').find('.theme-option');
    let $themeStyleChooser = $('#theme-style-chooser').find('.dt-color-option');
    let $layoutChooser = $('#layout-chooser').find('.choose-option__icon');
    ...
    let $toggleFixedHeader = $('#toggle-fixed-header');
    let $toggleFixedSidebar = $('#toggle-fixed-sidebar');
    let $layoutContainer = $('#sidebar-layout');
    let $sidebarLayoutHandle = $layoutContainer.find('.choose-option__icon');
    ...
    let $mailContacts = $('.contacts-list .dt-contact');
    let $userInfoCard = $('.user-info-card');
    ...
    $mailContacts.each(function () {
        ...
        $contact.hover(function () {
            let contactWidth = $contact.outerWidth(true);
            let positionValue = $contact.offset();
            let bodyHeight = $body.outerHeight(true);
            let bodyWidth = $body.outerWidth(true);

            if (bodyWidth > 767) {
                let userPic = $('.dt-avatar', $contact).attr('src');
                let userName = $('.dt-contact__title', $contact).text();
                ...
                let infoCardHeight = $userInfoCard.outerHeight(true);
    ...
    if ($('#user-menu-dropdown').length) {
        let userDorpdown = $('#user-menu-dropdown').find('.dropdown-menu').clone(true).appendTo("body");

        $('#user-menu-dropdown').on('show.bs.dropdown', function () {
            let $this = $('#user-menu-dropdown');
            userDorpdown.removeClass('show');
            if ($body.hasClass('dt-sidebar--folded')) {
                let positionValue = $this.find('.dropdown-toggle').offset();
                let dropDownHeight = $this.find('.dropdown-toggle').outerHeight(true);
    ...
    if (jQuery('body').hasClass('dt-sidebar--folded')) {
        if (!hoverEventExcuted) {
            ...
            jQuery(".dt-sidebar--folded ul.dt-side-nav > li.dt-side-nav__item").each(function () {
                let $currentMenuItem = jQuery(this);
                let $submenu = jQuery('.dt-side-nav__sub-menu', $currentMenuItem);
                let ps;
                ...
                $currentMenuItem.find('> a.dt-side-nav__link', $submenu).hover(function () {
                    ...
                    if ($submenu.length && jQuery('body').hasClass('dt-sidebar--folded')) {
                        ...
                        let menuItemWidth = $currentMenuItem.outerWidth(true);
                        let positionValue = $menuLink.offset();
                        let bodyHeight = $body.outerHeight(true);
                        let $submenuHeight = $submenu.outerHeight(true);
                        ...
                        let $menuLinkText = $menuLink.find('.dt-side-nav__text');
                        let menuItemWidth = $currentMenuItem.outerWidth(true);
                        let positionValue = $menuLink.offset();
                        let menuLinkHeight = $menuLink.outerHeight(true);
                        let $menuLinkTextHeight = $menuLinkText.outerHeight(true);
                        let offsetTop = positionValue.top + ((menuLinkHeight - $menuLinkTextHeight) / 2);
        ...
        if (!clickEventExcuted) {
            ...
            jQuery("ul.dt-side-nav__sub-menu li").on('click', function (e) {
                let $current_sm_li = jQuery(this);
                let $current_sm_li_parent = $current_sm_li.parent();
...
function changeTheme($theme, $style, $themeStylesheet, $init) {
    let $body = jQuery('body');
    let $logo = $body.find('.dt-brand__logo-img');
...
function activeLayoutHandle(layout) {
    let $layoutContainer = jQuery('#sidebar-layout');
...
function activeFixedStyle() {
    let $body = jQuery('body');
    let $toggleFixedHeader = jQuery('#toggle-fixed-header');
    let $toggleFixedSidebar = jQuery('#toggle-fixed-sidebar');
...
function init_indecator() {
    let $progressbar = jQuery('.dt-indicator-item__info:not(.complete)');

    $progressbar.each(function (index) {

        let $currentBar = $(this);
        let percentage = Math.ceil($currentBar.data('fill'));
        let maxVal = ($currentBar.data('max')) ? parseInt($currentBar.data('max')) : 100;
        let fillTypePercent = !!($currentBar.data('percent'));
        let textSufix = ($currentBar.data('suffix')) ? $currentBar.data('suffix') : '';
        ...
        if (percentage && percentage <= maxVal) {
            $({countNum: 0}).animate({countNum: percentage}, {
                ...
                step: function (now, tween) {
                    ...
                    let widthPct = Math.floor((pct * 100) / maxVal) + '%';
